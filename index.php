...
<body>
    ...
    <script>
        ...
        function showAddForm(table) {
            ...
            title.textContent = `Agregar ${getTableDisplayName(table)}`;
        ...
        function editSelected() {
            ...
            const item = filteredData[selectedRow];
            
            title.textContent = `Editar ${getTableDisplayName(currentTable)}`;
            ...
            fields.forEach(field => {
                ...
                if (field.type === 'textarea') {
                    ...
                    field.options.forEach(option => {
                        ...
                        opt.value = option;
                        opt.textContent = option;
                        opt.selected = item[field.name] === option;
                        input.appendChild(opt);
                    });
                } else if (field.type === 'checkbox') {
                    input = document.createElement('input');
                    input.type = 'checkbox';
                    input.checked = item[field.name] || false;
                } else {
                    input = document.createElement('input');
                    input.type = field.type;
                    input.value = item[field.name] || '';
        ...
        function deleteSelected() {
            ...
            if (confirm(`¿Estás seguro de que deseas eliminar "${itemName}"?`)) {
                // Encontrar el índice en los datos originales
                const originalData = mockData[currentTable];
                const originalIndex = originalData.findIndex(data => data.id === item.id);
                ...
                if (originalIndex > -1) {
                    ...
                    showMessage(`"${itemName}" ha sido eliminado correctamente.`, 'success');
                }
            }
        }
        ...
        function handleFormSubmit(e) {
            ...
            if (mode === 'add') {
                // Generar nuevo ID
                const maxId = Math.max(...mockData[table].map(item => item.id), 0);
                data.id = maxId + 1;
